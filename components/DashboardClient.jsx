...
export default function DashboardPage() {
  ...
  // Estados de Sincronização com o Servidor (Neon / Prisma)
  ...
  // Função para buscar dados do Servidor (Neon via API Prisma)
  const fetchServerData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (serverData && serverData.couple) {
          setState(serverData);
          setEditingBudgets(serverData.budgets || {});
          setPerson(serverData.couple?.person1 || "Bia");
          localStorage.setItem("app-financas-v6", JSON.stringify(serverData));
      ...
      console.warn("Servidor offline ou rota indisponível, usando cache local:", err);
      setSyncStatus("offline");
      return false;
    }
  };

  // Função para enviar dados atualizados para o Servidor (Neon via API Prisma)
  const syncToServer = async (dataToSync) => {
    setSyncStatus("syncing");
    try {
      const res = await fetch("/api/financas", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    // Tenta carregar dados atualizados do servidor na inicialização
    fetchServerData();
  }, []);
  ...
  const saveState = (newState) => {
    ...
    // Debounce para sincronizar com o servidor sem sobrecarregar requisições
    ...
    <div className="min-h-screen pb-24 max-w-md mx-auto bg-[#F5F6F4] text-gray-800 font-sans">
      ...
      <div className="p-4 bg-white border-b border-gray-200 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center gap-2">
          {/* Indicador de Sincronização com o Banco Neon/Prisma */}
          <button
            onClick={fetchServerData}
            title={
              syncStatus === "synced"
                ? `Sincronizado com o servidor (${lastSyncTime})`
                : syncStatus === "syncing"
                ? "Sincronizando..."
                : "Modo Offline (Toque para tentar reconectar)"
            }
            className={`flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full font-medium transition ${
              syncStatus === "synced"
